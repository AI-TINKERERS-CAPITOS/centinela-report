...
**AI agent to speed up the patient admission process in emergency rooms**

---

## 1. Project Description

Centinela is an AI agent that speeds up the admission process for patients arriving at the emergency room by ambulance. During triage, information travels verbally or on paper under time pressure, and data gets forgotten, repeated, or lost in the handoff to the doctor.

Centinela listens to the evaluation spoken aloud by the person in charge and automatically structures the case: vital signs, triage category, room assignment based on availability, and relevant history from the patient's medical record. The doctor reviews and confirms the information before it is saved, and once the case is closed, a confirmation email is sent as a record.

**Who it's for:** triage staff and emergency room doctors.

**Why context matters:** the agent acts at the exact moment when the most information gets lost and there is the least time to ask for it again — it is not a separate chatbot, but a layer that documents in real time inside the clinical process itself.
...
1. **Audio capture**: triage staff narrate the patient evaluation out loud, the same way they normally would when reporting to the doctor.
...
4. **Tool execution**: depending on what is detected, the model calls one or more functions (they are not sequential or mandatory in every turn; they trigger based on what is mentioned in the conversation).
5. **Form update**: each tool updates the form's state on screen in real time.
6. **Human confirmation (human-in-the-loop)**: the doctor reviews what the agent structured, corrects it if needed, and confirms.
7. **Closure & record**: once confirmed, the record is saved as final and a confirmation email is triggered with the complete case summary.
...
3. Sends that summary by email as a record of the process, closing the documentation cycle for the case.

---

## 8. Scope and Limitations (Hackathon)
...
## 9. Future Improvement: Facial Recognition for Patient & Insurance Identification

An ideal next step for Centinela would be adding **facial recognition** to automatically identify the patient on arrival and pull up their **insurance information** alongside their medical history, without needing anyone to state a name or ID out loud — which is especially valuable when the patient arrives unconscious, disoriented, or unable to communicate.
...
- Removes a dependency on the patient being able to speak or carry ID, which is common in emergency arrivals.
- Speeds up insurance verification, a step that today often causes delays or gets postponed until after initial treatment.
- Complements voice-based data capture instead of replacing it — Centinela would still document vitals and triage the same way, but identity resolution becomes automatic.
